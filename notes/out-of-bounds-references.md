...
This document catalogs all cases in the `Prosodia` project where code, configuration, or tests reference paths outside the `Prosodia` project root. These out-of-bounds references typically point to the shared `Reference/models/` directory at the umbrella workspace root or reference fallback paths in the developer's home directory.
...
* **Description:** Unit tests in the actor crate require loading raw TFLite model weights and configuration files. They navigate three levels up (`../../../Reference/models/...`) to read files from the shared `Reference/models/` directory:
  * `../../../Reference/models/matcha_stock.tflite`
  * `../../../Reference/models/sonora.tflite`
  * `../../../Reference/models/config.json`
* **Impact:** Running `cargo test` in a standalone checkout of the `Prosodia` repository will fail these tests unless a `Reference/models/` directory is manually populated at the parent directory level.
...
* **Description:** The Swift model managers discover models on-disk using a hardcoded search priority. These checks look outside the project bundle and bundle roots:
  * Default `modelsBase` config is initialized to `../Reference/models` (one directory above the Xcode project root).
  ...
  * Default fallback URL if the config file discovery fails assumes the umbrella workspace layout:
    * `~/Projects/Artificial-Humanity/Reference/models`
...
* **Description:** Contains documentation about the models layout:
  ```git
  # Models now live at the workspace root (../Reference/models), shared across subprojects.
  ```
...
1. **Test Environment Variables:** Update the Rust actor crate unit tests to look for a `TEST_MODELS_DIR` environment variable. If set, use it to locate model files; otherwise, fall back to the default `../../../Reference/models` relative path.
